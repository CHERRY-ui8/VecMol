...
def run_quality_evaluation(molecule_dir,
    ...
    molecules = load_molecules_from_npz(molecule_dir, use_sdf_bonds=use_sdf_bonds)
    if not molecules:
        print("没有找到有效的分子文件！")
        return None
...
def run_evaluation(config):
    ...
    # 从配置读取质量评估选项
    quality_eval_config = config.get('quality_evaluation', {})
    use_sdf_bonds = quality_eval_config.get('use_sdf_bonds', True)
