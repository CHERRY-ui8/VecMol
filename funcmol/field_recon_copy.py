...
def main(config: DictConfig) -> None:
    ...
    # 获取批量处理配置
    reconstruction_batch_size = config.get('reconstruction_batch_size', 1)
    if reconstruction_batch_size is None or reconstruction_batch_size < 1:
        ...
        for field_method in field_methods:
            try:
                ...
                if field_mode == 'gt_field':
                    ...
                    # 注意：gt_field模式下，每个样本需要独立的decoder（因为gt_coords不同）
                    # 所以gt_field模式不支持真正的批量处理，但可以批量调用gnf2mol
                    grid_size = config.get('dset', {}).get('grid_size', 9)
                    code_dim = config.get('encoder', {}).get('code_dim', 128)
                    
                    # 创建批量dummy codes
                    dummy_codes = torch.randn(actual_batch_size, grid_size**3, code_dim, device=batch_gt_coords[0].device)
                    
                    # 创建批量dummy decoder（包装多个样本的decoder）
                    class BatchDummyDecoder:
                        def __init__(self, converter, batch_gt_coords, batch_gt_types):
                            self.converter = converter
                            self.batch_gt_coords = batch_gt_coords
                            self.batch_gt_types = batch_gt_types
                        
                        def __call__(self, query_points, codes):
                            # query_points: [B, n_points, 3]
                            # codes: [B, grid_size**3, code_dim]
                            B = query_points.size(0)
                            batch_fields = []
                            for b in range(B):
                                field_b = self.converter.mol2gnf(
                                    self.batch_gt_coords[b].unsqueeze(0),
                                    self.batch_gt_types[b].unsqueeze(0),
                                    query_points[b:b+1]
                    ...
                    for i, sample in enumerate(batch_samples):
                        data = Data(
                            pos=sample.pos,
                            x=sample.x,
                            batch=torch.full((sample.pos.shape[0],), i, dtype=torch.long, device=sample.pos.device)
                        )
                    ...
                    with torch.no_grad():
                        codes = encoder(batch)  # [actual_batch_size, grid_size**3, code_dim]
                
                # 2.2 Reconstruct mol - 批量处理
                # 从配置中读取保存参数
                ...
                # 批量重建
                recon_coords, recon_types = converter.gnf2mol(**gnf2mol_kwargs)  # [actual_batch_size, n_atoms, 3]
                ...
                print(f"Error processing batch {batch_sample_indices} with {field_method}: {e}")
                import traceback
                traceback.print_exc()
                # Save error result for each sample in the batch
                for b in range(actual_batch_size):
                    sample_idx = batch_sample_indices[b]
                    gt_coords = batch_gt_coords[b]
                    gt_types = batch_gt_types[b]
