...
from vecmol.models.ema import ModelEma
...
from vecmol.dataset.dataset_code import create_code_loaders
...
class VecmolLightningModule(pl.LightningModule):
    """
    PyTorch Lightning module for training Vecmol (denoiser)
    """
    def __init__(self, config, enc, dec_module, code_stats, field_loader_train=None, field_loader_val=None):
        ...
        # Create Vecmol model
        self.vecmol = create_vecmol(config)
        ...
        with torch.no_grad():
            self.vecmol_ema = ModelEma(self.vecmol, decay=config["ema_decay"])
    ...
    def _get_multi_timestep_codes(self, x_0):
        ...
        """
        from vecmol.models.ddpm import q_sample, p_sample_x0
        
        device = x_0.device
        B = x_0.shape[0]
        diffusion_consts = self.vecmol.diffusion_consts
        ...
        if self.vecmol.diffusion_method == "ddpm_x0":
            # Model predicts x0 directly
            predicted_x0 = self.vecmol.net(x_t, t)
        else:
            # ddpm_epsilon: predict noise, then get x0 via one-step denoise
            predicted_x0 = p_sample_x0(self.vecmol.net, x_t, t, diffusion_consts, clip_denoised=False)
    ...
    def _training_step_ddpm(self, batch, batch_idx):
        ...
        # Update EMA model
        self.vecmol_ema.update(self.vecmol)
    ...
    def configure_optimizers(self):
        ...
        if self.joint_finetune_enabled:
            param_groups = [
                {"params": self.vecmol.parameters(), "lr": self.config["lr"], "weight_decay": self.config["wd"]}
            ]
            ...
            optimizer = AdamW(self.vecmol.parameters(), 
                             lr=self.config["lr"], 
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["vecmol_state_dict"] = vecmol_state_dict
        checkpoint["vecmol_ema_state_dict"] = vecmol_ema_state_dict
...
def main(config):
    ...
    model = VecmolLightningModule(model_config, enc, dec_module, code_stats, 
