...
\begin{figure}[t]
    \centering
    \vspace{-0.8cm}
    \includegraphics[width=\textwidth]{figures/overall_pipeline.pdf}
    \vspace{-0.3cm}
    \caption{
    Overview of the proposed molecular neural field framework.
    The architecture is organized into two rows.
    
    \textbf{Top row (from left to right):}
    (1) An input 3D molecule represented by atomic types and coordinates;
    (2) a deterministic encoder that converts the molecular graph into a continuous vector field (gradient field);
    (3) a neural field decoder that reconstructs the molecular structure from the vector field representation.
    
    \textbf{Bottom row (from left to right):}
    (4) the encoded vector field is projected into a latent space;
    (5) a diffusion model operating in the latent space learns the distribution of molecular fields;
    (6) generated latent samples are decoded back into vector fields and further reconstructed into valid 3D molecular structures.
    }
...
\subsection{Encoder: Molecule-to-Field Encoding}
...
\subsection{Decoder: Field Prediction with Equivariance}
...
Empirically, we find that this composite field definition
outperforms alternative formulations (e.g., Gaussian or LogSumExp fields),
...
\textbf{Stage 1: Neural Field Autoencoder.}
...
\textbf{Stage 2: Latent Diffusion Model.}
